...
class cls_pointsheet_overlap {
  ...
  // This function will read the Point Sheet file and determine if there is
  // a potential overlap of starting and ending dates/times.
  // At least one point sheet will be a department-level point sheet (fire, drill, etc.).
  function fct_compare_PS(){
    // Read Point Sheet header file for a point year.
    ...
    //  **** read row from pointsheet dataset;
    // 1. if this is the first row read, initially load the "keep" variables (used to compare for the next read);
    ...
    //    if there is data in "keep" variables, begin audit of PS headers and line numbers. Add array to all_data array if audit passes.
      ...
      if($work_psnumber_break == false){
        ...
        if($this->wrk_ending_date > $PSH['Starting_date']){
          ...
          if(!$work_is_previousrow_loaded){
            ...
            $work_array_PSD = $this->fct_load_PS_detail($this->wrk_pointsheet_nbr);  // function returns an array.
            $work_array_PSH["line_numbers"] = $work_array_PSD;  // Push PSD array onto PSH header (so an array within an array)
          ...
          $work_array_PSD = $this->fct_load_PS_detail($PSH['ID_Pointsheet']);  // function returns an array.
          $work_array_PSH["line_numbers"] = $work_array_PSD;  // Push PSD array onto PSH header (so an array within an array)
          array_push($work_array_keep_temp, $work_array_PSH);  // Now push PSH/PSD array onto the temp "overlap" array.
        }
  ...
  function fct_load_PS_detail($arg_PSnumber){
    ...
    foreach($return_hold as $row){
      array_push($array_psd,          $row['Line_nbr']);
      array_push($this->array_merged, $row['Line_nbr']);
    }
    return $array_psd;
  }
